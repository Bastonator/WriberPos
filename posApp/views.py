...
def save_category(request, pk):
    ...
    try:
        if (data['id']).isnumeric() and int(data['id']) > 0 :
            ...
            save_category = Category(name=data['name'], description = data['description'],status = data['status'], branch_owner = branch)
            save_category.save()
        resp['status'] = 'success'
    ...
    if len(check) > 0 :
        ...
        try:
            if (data['id']).isnumeric() and int(data['id']) > 0 :
                ...
                save_product = Products(code=data['code'], category_id=category, name=data['name'], description = data['description'], price = float(data['price']),status = data['status'],stock= int(data['stock']) ,branch_owner = branch)
                save_product.save()
            resp['status'] = 'success'
...
def save_pos(request, pk):
    ...
    try:
        sales = Sales(code=code, sub_total = data['sub_total'], tax = data['tax'], tax_amount = data['tax_amount'], grand_total = data['grand_total'], tendered_amount = data['tendered_amount'], amount_change = data['amount_change'], branch_owner = branch).save()
